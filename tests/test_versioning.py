...
from drain_swamp._version import (
    __version__,
    __version_tuple__,
)
...
class PackageVersioning(unittest.TestCase):
    ...
    def test_setuptools_scm_version_file(self):
        ...
        if len(__version_tuple__) == 3:
            ...
            self.assertEqual(__version__, f"{ver_long}+{ver_git}")
...
class SemVersioning(unittest.TestCase):
    ...
    def test_tag_version(self):
        """Get latest tagged release if it exists"""
        with (
        ...
        # get app name from git
        cmd = []
        expected = "0.0.1"
        with (
            patch(
                f"{g_app_name}.version_semantic._get_app_name",
                return_value=g_app_name.replace("-", "_"),
            ),
            patch(
                "subprocess.run",
                return_value=subprocess.CompletedProcess(
        ...
        # Simulate getting a tagged version from git
        next_versions = (
            None,
            "",
        )
        ...
        for next_version in next_versions:
            with (
                patch(
                    f"{g_app_name}.version_semantic._current_tag",
    ...
    def test_version_clean(self):
        ...
        actual_ver = sv.version_clean(kind, package_name=g_app_name)
        ...
        with self.assertRaises(ValueError):
            sv.version_clean(kind, package_name=g_app_name)
        ...
        with (
            ...
            actual = sv.version_clean(kind, package_name=g_app_name)
            self.assertEqual(actual, expected)
        ...
        with (
            patch(f"{g_app_name}.version_semantic._tag_version", return_value=None),
            patch(f"{g_app_name}.version_semantic._current_version", return_value=None),
            self.assertRaises(SetuptoolsSCMNoTaggedVersionError),
        ):
            actual_ver = sv.version_clean(kind, package_name=g_app_name)

        # current aliases. Both None --> SetuptoolsSCMNoTaggedVersionError
        kinds = (
            "current",
            "now",
        )
        for kind in kinds:
            ...
            with (
                patch(f"{g_app_name}.version_semantic._tag_version", return_value=None),
                patch(
                    f"{g_app_name}.version_semantic._current_version", return_value=None
                ),
                self.assertRaises(SetuptoolsSCMNoTaggedVersionError),
            ):
                actual_ver = sv.version_clean(kind, package_name=g_app_name)
